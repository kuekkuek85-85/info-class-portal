...
const SYSTEM = [
  ...
  "- 반·분반 전체의 감정을 물으면(예: '1반에서 감정적으로 눈여겨볼 학생', '마음 톡톡 목요일 1기에서 눈여겨볼 학생') 학생을 하나씩 돌지 말고 classEmotions 를 한 번 불러 그 결과로 판단한다. 정규수업이면 class(1~4), 분반이면 group 에 열쇠나 이름('목요일 1기')을 넘긴다. 절대 '범위가 넓다'며 되묻지 말고, 분반 이름이 있으면 그대로 group 에 넣어 부른다.",
  "- 동료 피드백을 많이 받은 작품을 물으면(예: '4반에서 피드백 많이 받은 작품 추천') 작품을 하나씩 돌지 말고 topFeedback 을 한 번 불러 그 순위로 답한다.",
