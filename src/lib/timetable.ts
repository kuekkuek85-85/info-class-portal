...
const SCHEDULES: Schedule[] = [
  ...
  /*
   * 정상 시간표 (2026-08-22 ~ 학기말).
   *
   * ⚠ 아래 시각은 일반적인 중학교 45분 수업 기준으로 **가정한 값**이다.
   *   학교 실제 시정과 다르면 코드가 너무 일찍/늦게 만료된다. 개학 후 실제 시정표로
   *   반드시 한 번 대조할 것.
   *
   * 구간을 비워 두는 쪽이 더 나쁘다 — 등록되지 않은 날짜는 isPeriodOver() 가 항상 false 라
   * 자동 만료가 오류도 경고도 없이 조용히 꺼진다 (PRD 10장 D3).
   */
  {
    from: "2026-08-22",
    to: "2027-02-28",
    label: "정상 시간표 (가정값 — 실제 시정표로 확인 필요)",
    periods: [
      ...
      // 점심 12:20~13:10
      { period: 5, start: "13:10", end: "13:55" },
