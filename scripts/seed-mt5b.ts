 ...
 * (4분 41초, 한글자막 없음). 시상 **직후**에 재생한다 — "여러분이 방금 한 게
 * 회복탄력성이라는 근육" 을 개념으로 못박는 자리다.
 */
const VIDEO_URL = "https://www.youtube.com/watch?v=0T8IziNtb18";
...
const WORKSHEET: WorksheetQuestion[] = [
  ...
  },
  {
    ...
    phase: "emotion",
    /*
     * 시상 직후 회복탄력성 프레이밍. 영상②는 이 안내 다음, 정리 단계에서 재생한다
     * (교사가 video 단추를 누른다). 방금 한 일에 개념 이름을 붙여 준다.
     *
...
const PLAN: Omit<LessonPlan, "id" | "createdAt" | "updatedAt"> = {
  ...
  progress: {
    ...
    body:
      ...
      "③ 시상·마무리 — 부문별 명예 실패자에게 박수, ‘회복탄력성’ 영상으로 오늘을 정리하고,\n" +
      "   자기 영역을 한 문장으로 닫기\n\n" +
      "반응 이모지 = 시상 부문이에요:\n" +
  ...
  /*
   * 프레이밍 영상 ②. 시상 뒤에 재생한다. 한글자막이 없어 body 요약으로 흐름을 짚는다.
   *
   * ★ [2:36] 반전을 body 에 반드시 넣는다. 잡스·조던·롤링 사례 셋만 보여주고 끝내면
   * 메시지가 "실패는 (반드시) 성공의 어머니" 로 정반대로 뒤집힌다. 회복탄력성은 성공을
   * 보증하는 보상이 아니라, 결과와 무관하게 다시 일어서는 근육이라는 선을 긋는다.
   */
  video: {
    heading: "회복탄력성 — 마음 근육을 키우고 싶어 (EBS 지식채널e · 4분 41초)",
    body:
      "실수한 나, 그런 나를 또 자책하는 나… 여기서 다시 일어서는 힘이 ‘회복탄력성’ 이에요.\n" +
      "어원은 농구공의 탄성 — 바닥을 칠수록 튀어 오르는 힘이죠. 잡스·조던·롤링도 숱하게\n" +
      "넘어졌고, 회복탄력성은 타고나는 게 아니라 ‘마음의 근육’ 이라 노력하면 강해집니다.\n\n" +
      "★ 다만 [2:36] — 사례 셋은 ‘실패하면 반드시 큰 성공으로 보상받는다’ 는 약속이 아니에요.\n" +
      "회복탄력성은 성공 보증이 아니라, 결과가 어떻든 ‘다시 일어서는 근육’ 그 자체예요.\n\n" +
      "키우는 법 넷: 감사일기 · 명상 · 도움을 주고받는 관계 · 잘 자고 쉬고 움직이기.\n" +
      "그리고 가장 기본은 — 나 자신을 용서하기. “오늘, 나를 포기하지 않았으니.”",
    url: VIDEO_URL,
  },

  /*
   * 영상 중 화면에 띄울 생각거리. [2:36] 반전을 물음으로도 한 번 더 짚는다.
   */
  videoPrompts: [
    "회복탄력성은 왜 ‘타고나는 재능’ 이 아니라 ‘근육’ 이라고 할까?",
    "잡스·조던·롤링처럼 성공하지 못하면, 다시 일어선 건 의미가 없을까?",
    "오늘 실패를 노래로 자랑한 것은 이 네 가지 중 무엇에 가까울까?",
  ],

  /*
   * 마음일기 — 세 번째 문항을 영상의 ‘감사일기’ 로 잇되, 조건을 바꾼다.
   * ‘좋았던 일 쓰기’ 가 아니라 **안 풀린 하루에서 그래도 건질 것 찾기** 로.
   * reflectionPublic 은 반드시 false — 마음일기는 비공개 유지, 갤러리로 안 나간다.
   */
  reflectionQuestions: [
  ...
  phaseLabels: {
    ...
    build: "지난 시간 내 노래",
    gallery: "실패 노래 자랑대회",
    // 영상②는 시상 뒤에 교사가 눌러 재생한다(회복탄력성 프레이밍)
    video: "회복탄력성 영상",
    emotion: "시상·마무리",
    reflection: "마음일기",
  },
