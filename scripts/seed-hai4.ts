...
const WORKSHEET: WorksheetQuestion[] = [
  ...
  },
  {
    ...
    phase: "emotion",
    /*
     * 2·3차시와 같은 키 — 이 칸이 곧 "앱 링크 제출함" 이다.
     *
     * 교사 대시보드·미리 피드백·갤러리가 모두 이 build_url 하나를 보고 "링크를 냈나"를
     * 판정한다(teacher/dashboard 의 LINK_KEY). 그래서 링크 칸은 이것 하나뿐이어야 하고,
     * 별도 제출 폼을 따로 만들면 안 된다.
     *
     * 지난 시간에 낸 학생은 이 칸에 그 주소가 이미 채워져 있어 수정하는 자리가 되고,
     * 2·3차시에 못 낸 학생(이 반에 있을 수 있다)은 오늘 여기서 처음 제출한다.
     * 문구는 두 경우 모두 어색하지 않게 "내기/고치기"를 함께 담는다.
     * https:// 자동 보정은 저장 시 normalizeUrl 이 처리한다. 같은 작품 문서라
     * 다음 시간(5차시 동료 검토)에 이 최신 주소가 그대로 열린다.
     */
    label: "내 앱(화면) 링크를 여기에 내 주세요",
    hint:
      "캔바 오른쪽 위 [게시] 를 눌러 나온 주소를 붙여 넣으세요.\n" +
      "처음 내는 거예요 → 게시해서 나온 주소를 그대로 붙여 넣으면 됩니다.\n" +
      "지난 시간에 이미 냈어요 → 아래에 그 주소가 들어 있어요. 게시를 다시 해서 주소가\n" +
      "바뀐 경우에만 새것으로 고치면 됩니다 (안 바뀌었으면 그대로 두세요).\n" +
...
const PLAN: Omit<LessonPlan, "id" | "createdAt" | "updatedAt"> = {
   ...
   * 고치는 경험에서 새로 알게 된 것·느낀 것을 열어 놓고 쉽게 묻는다 (교사 확정 문구).
   */
  reflectionQuestions: [
    "오늘 만든 것을 고쳐 보면서 새로 알게 된 점이나 느낀 점을 적어 보세요.",
  ],
