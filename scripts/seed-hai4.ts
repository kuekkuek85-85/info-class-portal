 ...
 * 그 칸이 곧 그 답이다.
 *
 * ## 오늘의 진짜 벽 — Canva 는 저장을 못 한다
 *
 * 많은 아이디어가 "신청을 받는다 / 기록을 쌓는다 / 외부 정보를 불러온다" 인데,
 * Canva 코드 앱은 서버도 데이터베이스도 없다. 그래서 오늘의 핵심 기술 메시지는
 * **"저장이 필요한 기능은 미리 넣은 예시 데이터로 흉내 내라"** 이다. 반례로
 * 키 없이 되는 진짜 무료 API(Open-Meteo 날씨)도 하나 알려 준다. 이 둘을 읽고
 * 무엇을 어떻게 고칠지 스스로 정하게 한다.
 *
 * ## 세션은 열지 않는다
 *
...
const WORKSHEET: WorksheetQuestion[] = [
   ...
   */
  {
     ...
     * 오늘의 핵심 팁.
     *
     * Canva 코드 앱은 서버·DB 가 없어서 "저장"이 안 된다. 신청·기록·순위처럼 쌓이는
     * 기능을 넣으면 새로고침하는 순간 다 날아간다. 그래서 진짜로 저장하는 대신,
     * 미리 넣어 둔 예시 몇 개로 "이렇게 쌓인다"를 보여주게 한다.
     */
    label: "고치기 팁 ① — 저장이 필요한 기능은 ‘미리 넣은 예시’로 흉내 내세요",
    hint:
      "Canva 앱에는 서버가 없어서, 사람들이 넣은 내용을 진짜로 저장하지는 못해요.\n" +
      "신청 받기·기록 쌓기·순위표 같은 건 새로고침하면 사라집니다.\n\n" +
      "그럴 땐 진짜 저장 대신 예시를 미리 2~3개 넣어 두세요.\n" +
      "· 신청 앱 → 신청 목록에 ‘김○○ · 3반 · 축구’ 같은 예시가 이미 들어 있게\n" +
      "· 기록 앱 → 오늘 기록 위에 지난 기록 몇 개가 이미 쌓여 있게\n" +
      "· 순위 앱 → 1~3등이 이미 채워져 있게\n" +
      "보는 사람은 ‘아, 이렇게 쌓이는 앱이구나’ 를 바로 알 수 있어요. 발표에는 이걸로 충분합니다.",
    kind: "note",
    maxLength: 0,
  },
  {
    ...
    phase: "grill",
    /*
     * 반례. "저장·외부연결은 다 안 된다"로 굳지 않게, 키 없이 되는 진짜 API 를 하나 준다.
     * Open-Meteo 는 가입도 API 키도 없이 날씨를 준다 — 중1이 프롬프트에 붙일 수 있는
     * 거의 유일한 예다. copyText 로 주소를 그대로 복사하게 둔다.
     */
    label: "고치기 팁 ② — 진짜로 되는 무료 정보도 있어요 (날씨)",
    hint:
      "모든 외부 정보가 막힌 건 아니에요. 날씨는 가입도 키도 없이 진짜로 불러올 수 있어요.\n" +
  ...
  },
  {
    ...
    hint:
      ...
      "예) 신청 목록에 예시 3개를 미리 넣어서, 신청이 쌓이는 것처럼 보이게 했어요",
    kind: "long",
    maxLength: 200,
  },
];
...
const PLAN: Omit<LessonPlan, "id" | "createdAt" | "updatedAt"> = {
  ...
  progress: {
    ...
    body:
      ...
      "오늘의 큰 팁 하나: 캔바 앱은 ‘저장’ 을 못 해요. 신청·기록처럼 쌓이는 기능은\n" +
      "예시를 미리 몇 개 넣어서 ‘이렇게 쌓인다’ 를 보여주면 됩니다. 자세한 건 안에서 설명해요.",
    url: "",
  },
  assessment: empty(),
  video: empty(),
