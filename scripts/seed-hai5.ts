 ...
 *  - 서버가 필수 2편을 학번순 순환으로 배정하고(assignPeers), 학생이 자유 1편을
 *    더 골라 = **한 명이 3편**. 모든 앱이 정확히 두 번씩 배정돼 아무도 빠지지 않는다.
 *  - 카드 밑 두 칸짜리 피드백 폼(gallery-view 의 FeedbackForm)은 그대로 두고,
 *    **문구만** 앱 검토에 맞게 바꾼다(feedbackPrompts). 폼 필드 자체는 코드 변경이라
 *    건드리지 않는다 — 기준은 안내(note)로 주고 기존 칸을 재사용한다.
 *
...
const WORKSHEET: WorksheetQuestion[] = [
   ...
   */
  {
    ...
    hint:
      "위쪽 [앱 감상] 탭을 누르면 친구들의 앱이 뜹니다.\n" +
      "‘꼭 보기’ 표시가 붙은 2명은 반드시 보고, 자유롭게 1명을 더 골라 = 모두 3명을 봅니다.\n" +
      "선생님이 나눠 준 것이라, 내 앱도 다른 친구 2명이 꼭 보게 돼 있어요.\n\n" +
      "볼 때 이 네 가지를 확인해 주세요\n" +
      "① 무엇을 하는 앱인지 첫 화면에서 한눈에 알 수 있나요?\n" +
      "② 버튼·화면이 이해되나요? 눌러 보고 싶은가요? (진짜 눌러 보세요)\n" +
      "③ 좋은 점 한 가지 — 무엇이 좋았는지 구체적으로\n" +
      "④ 이렇게 하면 더 좋겠다 한 가지 — 깎아내리지 말고, 도움이 되게\n\n" +
      "카드 밑 두 칸에 ③좋은 점과 ④더 좋을 점을 남기면 됩니다. 이모지도 눌러 주세요.\n" +
      "‘잘했어요’ 한 마디로 끝내지 말고, 실제로 눌러 본 것에서 한 가지씩 적어 주세요.\n" +
...
const PLAN: Omit<LessonPlan, "id" | "createdAt" | "updatedAt"> = {
  ...
  progress: {
    ...
    body:
      ...
      "② 친구 앱 검토 — ‘꼭 보기’ 2명 + 자유 1명 = 모두 3명의 앱을 보고 한마디씩 남깁니다 (약 14분)\n" +
  ...
  activity: {
     ...
     * 앱 검토에 필요한 것만: 한 줄 소개(mvp_one)와 앱 링크(build_url).
     * build_url 은 URL 칸이라 카드에서 "눌러서 작품 보기" 로 뜨고, 눌러 들어가면 실제
     * 앱이 새 창으로 열린다(card-news). fix1·grill_a2 같은 자기 성찰 칸은 안 나간다.
     */
    galleryAnswerKeys: ["mvp_one", "build_url"],

    /*
     * 친구 앱에 남기는 두 칸의 문구를 앱 검토에 맞게 바꾼다.
     * 폼 필드 자체(gallery-view 의 FeedbackForm)는 그대로 두고 label·placeholder 만 바꾼다 —
     * 코드 변경 없이 재사용. 첫 칸은 관찰(좋은 점), 둘째 칸은 도움이 되는 제안이다.
     * 기준 ①②(무슨 앱인지·써 보고 싶은지)는 위 검토 기준 안내에 두었다.
     */
    feedbackPrompts: {
      found: {
        label: "이 앱에서 좋은 점 한 가지 — 눌러 보고",
        placeholder: "예) 버튼을 누르니 진짜로 오늘 급식이 떠서 좋았어요",
      },
      question: {
        label: "이렇게 하면 더 좋겠다 — 한 가지만",
...
async function main(): Promise<void> {
  ...
  console.log("  ※ 동료 검토는 gallery.ts 를 그대로 재사용: 필수 2편(학번순 순환) + 자유 1편 = 한 명당 3편, 모든 앱 정확히 2번 배정");
  console.log("  ※ 피드백 폼 문구만 앱 검토에 맞춰 바꿨습니다 (feedbackPrompts) — 폼 필드 자체는 그대로.");
  console.log("  ※ 친구에게 보이는 칸은 mvp_one·build_url 둘뿐 (galleryAnswerKeys) — 자기 성찰 칸은 안 나갑니다.");
