 ...
 *  - 서버가 필수 2편을 배정하고(assignPeers), 학생이 자유 1편을 더 골라 = **한 명이 3편**.
 *    모든 앱이 정확히 두 번씩 배정돼 아무도 빠지지 않는다.
 *  - 이 분반은 여러 반이 섞여 있어, 배정을 **진짜 랜덤(peerAssign: "random")** 으로 켠다.
 *    세션 시드로 한 번 섞은 순서에 같은 순환을 얹어, 반이 섞이면서도 균등·안정·전원 커버가
 *    유지된다. 플래그를 안 켠 다른 차시는 기존 학번순 순환 그대로다 (회귀 없음).
...
async function main(): Promise<void> {
  ...
  console.log("  ※ 동료 검토는 gallery.ts 를 그대로 재사용: 필수 2편 + 자유 1편 = 한 명당 3편, 모든 앱 정확히 2번 배정");
  console.log("  ※ 배정은 진짜 랜덤 (peerAssign: \"random\") — 세션 시드로 섞어 반 쏠림을 풀되 균등·안정·전원 커버 유지. 다른 차시는 학번순 순환 그대로.");
