/**
 * 「인간과 인공지능」 6차시 · 화요일 1기 수업을 연다.
 *
 *   node --env-file=.env.local scripts/open-hai6-tue1.ts
 *   node --env-file=.env.local scripts/open-hai6-tue1.ts 2026-09-22 1
 *
 * open-hai5-tue1.ts 를 복제해 6차시로 맞춘 것이다. 교사 화면에서도 만들 수 있지만,
 * 분반 수업은 고를 것이 많아(분반·차시·날짜·교시) 수업 직전에 손으로 고르다 틀리기 쉽다.
 *
 * 날짜·교시는 인자로 바꿀 수 있다 (기본 2026-09-22 화요일 1교시). 목요일 1기는 사본
 * open-hai6-thu1.ts 를 쓴다 (seed-hai6 의 groups 표: hai-thu-1 = 목요일 1기 = CLASS_NO 3).
 *
...
/**
 * 안 쓰이는 두 자리 코드를 고르고 **예약까지 한다** (open-hai5 와 같은 이유).
 */
...
async function main(): Promise<void> {
  ...
  if (plans.empty) {
    console.error("✗ 106차시 계획이 없습니다. 먼저 scripts/seed-hai6.ts 를 돌리세요.");
    process.exit(1);
  }
  ...
  console.log(
    `  서로 구경하기  ${act?.galleryEnabled ? "켬" : "끔"} · 배정 ${act?.peerAssign ?? "cyclic"}` +
      ` ${act?.peerCount ?? 2}편` +
