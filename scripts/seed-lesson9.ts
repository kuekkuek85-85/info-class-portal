...
const PLAN: Omit<LessonPlan, "id" | "createdAt" | "updatedAt"> = {
   ...
   *
   * 「왜 도움이 되는지」 를 같이 묻는 것이 핵심이다. 그것이 없으면 "비밀번호를 잘
   * 관리하겠다" 같은 다짐으로 끝나는데, 11차시 루브릭이 상과 중을 가르는 자리가 바로
   * **근거** 다. 오늘 그 형식을 한 번 써 보게 한다.
   */
  reflectionQuestions: [
    "개인정보를 지키려고 오늘부터 바꿀 것을 하나만 정해 보세요. 무엇을 어떻게 바꿀 건가요? 그리고 그것이 왜 도움이 된다고 생각하나요?",
  ],
