 ...
 *
 * 각 장면은 겪어 본 뒤에 **개념과 수칙**을 편다 — 교사가 그것을 짚어 가며 설명한다.
 * 먼저 당해 보고, 그 다음에 "방금 그게 이거였다" 로 정의를 읽는 순서다.
 *
 * 보이스피싱은 화면 안에서 재현이 안 되므로(전화다) 개념 카드로만 둔다.
 *
...
const SCENES: ScamScene[] = [
  ...
  /*
   * 보이스피싱은 겪을 수가 없다 — 전화라 화면 안에서 재현이 안 된다. 그래서 개념 카드로만
   * 둔다. 영상은 하이퍼링크로 걸어, 교사가 필요할 때 새 창으로 연다.
   */
  {
    mode: "info",
    title: "⑤ 보이스피싱 — 전화로 낚는다",
...
const PLAN: Omit<LessonPlan, "id" | "createdAt" | "updatedAt"> = {
  ...
  /*
   * 실천 수칙 그림을 문항 옆에 둔다 (슬라이드 16).
   *
   * 방안을 백지에서 떠올리라고 하면 "비밀번호 바꾸기" 하나 쓰고 멈춘다. 아홉 칸짜리
   * 수칙 그림을 옆에 두면 그것을 보고 자기 것으로 골라 쓴다 — 베끼는 게 아니라 고르는 것.
   */
