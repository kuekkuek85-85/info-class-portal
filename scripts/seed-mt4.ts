...
const WORKSHEET: WorksheetQuestion[] = [
  // ── ① 실패를 노래로 만들기 (build) ───────────────────────────
  {
     ...
     */
    label: "① Suno 열기 — 반드시 Edge 로! 먼저 눌러 두세요 (로그인이 오래 걸려요)",
    hint:
      "⚠️ 오늘은 크롬(Chrome) 말고 반드시 Edge(파란-초록 소용돌이 아이콘, 마이크로소프트 엣지) 로 여세요.\n" +
      "   Suno 는 크롬에서 막혀요. 뒤에 나오는 Canva 도 같은 Edge 창에서 하면 로그인이 이어져요.\n\n" +
  ...
  },
  {
    ...
    hint:
      ...
      "[복사하기] 로 붙여 넣으면 됩니다(Suno 를 연 그 Edge 창에서, 같은 학교 계정 — 이미 로그인돼 있으면 바로 들어가져요).\n\n" +
...
const PLAN: Omit<LessonPlan, "id" | "createdAt" | "updatedAt"> = {
  ...
  progress: {
    ...
    body:
      ...
      "오늘은 Suno(노래)와 Canva(그림) 둘 다 로그인해요 — 반드시 Edge(마이크로소프트 엣지) 로 여세요. Chrome 에서는\n" +
      "Suno 가 막혀요. 둘 다 같은 학교(Microsoft) 계정이라 한 Edge 창에서 하면 대체로 바로 이어져요.\n" +
      "노래는 생성에 몇 분 걸리니 나온 곡부터 먼저 듣고, 안 나온 곡은 마지막에 다시 들러요.\n" +
...
async function main(): Promise<void> {
  ...
  console.log("\n[교사] 브라우저: Suno·Canva 둘 다 반드시 Edge 로 — Chrome 에서는 Suno 가 막힙니다(화요일 1기 실수업 확인). 학생 안내(① Suno 열기·오늘 할 일)에 반영됨.");
  console.log("[교사] Suno·Canva 둘 다 학교(MS) 계정 로그인 — 이중 로그인이라 짝당 1계정·도입 병렬 권장, 수업 전 사전 테스트. 시상·회복탄력성 영상②·마무리 문장은 활동지에서 제거됨(아래 보고 참조).");
