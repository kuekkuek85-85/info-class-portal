 ...
 * 판정 보기 셋.
 *
 * "곧 / 나중에 / 사람이 계속" 으로만 두면 중1은 근거 없이 찍는다. 보기 문구 안에
 * **판단 기준을 넣어 두면** 고르는 순간 이유를 함께 고르게 되고, 따로 설명할 필요가 없다.
 * 기준은 결국 "알고리즘으로 쓸 수 있는가" 다 — 순서가 정해져 있나, 상황마다 판단이 필요한가.
 *
 ...
 */
const VERDICTS = [
  "순서가 정해져 있어서 — 인공지능이 곧 가져간다",
  "인공지능과 나눠서 한다 — 인공지능이 하고 내가 고친다",
  "상황 따라 판단해야 해서 — 사람이 계속한다",
];
...
function taskFields(n: number): WorksheetQuestion[] {
  return [
    ...
    },
    {
      key: `task${n}_why`,
      label: `↳ 왜 그렇게 봤나요?`,
      hint: n === 1 ? "예) 순서가 늘 똑같아서 로봇 팔이 이미 하고 있어요" : "",
      kind: "text",
      maxLength: 80,
    },
  ];
...
const PLAN: Omit<LessonPlan, "id" | "createdAt" | "updatedAt"> = {
   ...
   */
  reflectionQuestions: [
    "오늘 내 직업을 분해해 보니, 어떤 조각이 인공지능에게 넘어가고 어떤 조각이 남던가요?\n그것 때문에 무엇을 바꾸기로 했는지 함께 적어 주세요.",
  ],
  ...
  activity: {
    ...
    feedbackPrompts: {
      ...
      question: {
        label: "친구의 판정 중에 “정말 그럴까?” 싶은 게 있었나요?",
    ...
    worksheet: [
       ...
       */
      {
        ...
        hint: "복잡한 것을 다룰 수 있는 크기로 쪼개는 것 — 우리가 배운 그 분해예요.\n하나씩 쪼갠 다음, 조각마다 인공지능이 가져갈지 따져 봅니다.",
        kind: "note",
        maxLength: 0,
      },
      ...taskFields(1),
      ...
      {
        key: "quote",
        label: "인공지능에게 물어봤을 때, 가장 마음에 걸린 한 문장",
        /*
         * "요약하지 말고" 가 핵심이다. 요약하게 두면 학생이 무의식적으로 순화한다.
         * 원문이 남아야 아프고, 아파야 다음 칸을 진지하게 쓴다.
         */
        hint: "요약하지 말고 그대로 옮겨 적어 주세요.",
        kind: "long",
        maxLength: 200,
      },
      /*
      ...
      {
        key: "synergy",
        label: "인공지능이 가져갈 조각 — 그 도구를 내가 쓰면 무엇을 더 할 수 있나요?",
        hint: "예) 손질을 로봇이 해 주면 나는 새 메뉴를 더 만들 수 있어요",
        kind: "text",
        maxLength: 100,
      },
      {
        key: "strength",
        label: "사람이 남는 조각 — 그래서 더 중요해질 내 강점은?",
        hint: "예) 손님이 무엇을 좋아할지 읽어 내는 눈",
        kind: "text",
        maxLength: 100,
      },
      /*
