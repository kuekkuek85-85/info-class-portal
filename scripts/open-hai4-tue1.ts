/**
 * 「인간과 인공지능」 4차시 · 화요일 1기 수업을 연다.
 *
 *   node --env-file=.env.local scripts/open-hai4-tue1.ts
 *
 * open-hai3-tue1.ts 를 복제해 4차시로 맞춘 것이다. 교사 화면에서도 만들 수 있지만,
...
/** 화면에 안 보이는 데이터 통 번호. 2·3차시와 같아야 지난 답이 열린다 */
const CLASS_NO = 1;
...
async function main(): Promise<void> {
  ...
  if (plans.empty) {
    console.error("✗ 104차시 계획이 없습니다. 먼저 scripts/seed-hai4.ts 를 돌리세요.");
    process.exit(1);
  }
