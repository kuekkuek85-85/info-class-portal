...
const PLAN = {
  ...
  activity: {
     ...
     * 감상 화면 왼쪽 필터.
     *
     * 기본값(특성 · 장소)을 그대로 두면 이 차시에서는 아무것도 거르지 못한다 —
     * 고른 특성도 그린 장소도 없다. 누르면 목록이 그대로라 학생은 고장으로 읽는다.
     *
     * 대신 친구들이 적어 낸 직업 이름을 그대로 항목으로 쓴다. "번역가를 사라질 쪽에
     * 적은 친구들" 을 모아 보는 것이 이 활동에서 가장 하고 싶은 일이다.
     *
     * 여기서는 글자가 같은 것만 묶인다. 뜻으로 묶는 것(교사 = 선생님)은 AI 를 부르는
