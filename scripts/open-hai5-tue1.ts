/**
 * 「인간과 인공지능」 5차시 · 화요일 1기 수업을 연다.
 *
 *   node --env-file=.env.local scripts/open-hai5-tue1.ts
 *   node --env-file=.env.local scripts/open-hai5-tue1.ts 2026-09-15 1
 *
 * open-hai4-tue1.ts 를 복제해 5차시로 맞춘 것이다. 교사 화면에서도 만들 수 있지만,
 * 분반 수업은 고를 것이 많아(분반·차시·날짜·교시) 수업 직전에 손으로 고르다 틀리기 쉽다.
 *
 * 날짜·교시는 인자로 바꿀 수 있다 (기본 2026-09-15 화요일 1교시). 다른 기(예: 목요일 1기)를
 * 열 때는 GROUP_KEY·GROUP_LABEL·CLASS_NO 를 그 분반 값으로 바꾼 사본을 쓴다
 * (seed-hai5 의 groups 표: hai-thu-1 = 목요일 1기 = CLASS_NO 3).
 *
 * ## 여기서 반드시 지켜야 하는 두 가지
 *
...
/** 화면에 안 보이는 데이터 통 번호. 2·3·4차시와 같아야 지난 답·앱이 열린다 */
const CLASS_NO = 1;
...
async function main(): Promise<void> {
  ...
  if (plans.empty) {
    console.error("✗ 105차시 계획이 없습니다. 먼저 scripts/seed-hai5.ts 를 돌리세요.");
    process.exit(1);
  }
