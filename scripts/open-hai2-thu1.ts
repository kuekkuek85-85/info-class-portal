 ...
 *
 *  3. **꼭 필요한 기능 ②③ 을 뺀다.** MVP 의 요점이 "제일 작게" 인데 기능을 셋 요구하는
 *     것 자체가 모순이다. ③ 은 22명 중 18명만 썼다 — 시간 압박의 신호다.
 *     프롬프트 서식에서도 그 두 자리를 빼야 한다. 안 그러면 빈칸이 들어간 문장이 나간다.
 *
 *  4. **AI 검토와 서로 구경하기를 뺀다.** 둘 다 만들어진 링크가 있어야 성립하는데
 *     화요일에 링크가 3개였다. 회고 한 줄(will_fix)만 남겨 grill 단계를 그것 하나로 만든다.
 *
 * 45분 배분 — 로그인 10 · 오늘 할 일 2 · 문제 찾기 6 · 꼭 필요한 것 5 · 만들기 16 ·
 *             다음에 고칠 것 3 · 여유 3
 */
...
async function main(): Promise<void> {
  ...
  for (const q of original) {
    ...
    // 기능 ②③ 을 뺐으므로 프롬프트 서식에서도 그 자리를 지운다
    if (next.key === "build_prompt") {
      next.prefillTemplate = "{mvp_one} 을(를) 만들어줘.\n꼭 필요한 기능은 {mvp_must1} 이야.";
    }
