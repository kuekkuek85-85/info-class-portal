// ⚠ 임시 번호(교사 확정 대기). 원래 12차였으나 오늘 블록-진단 12차로 교체되어 파킹.
//   새 12와 겹치지 않게 121 로 둠. 교사 별도 지시 전엔 재시드하지 않는다.
/**
 * 12차시(파킹, LESSON_NO=121) 차시 계획 등록 — 「프로그래밍 시작 · 도우미 선발」.
 *
 *   node --env-file=.env.local scripts/seed-lesson12-python-helper.ts
 *   node --env-file=.env.local scripts/seed-lesson12-python-helper.ts --force
 *
 * ## 왜 이 시간이 있나 (교사 확정)
 *
...
const LESSON_NO = 121; // 임시 파킹 번호 (원래 12 — 블록-진단 12로 교체됨)
