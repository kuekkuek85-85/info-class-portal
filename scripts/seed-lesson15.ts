/**
 * 15차시 차시 계획 등록 — 「피지컬 컴퓨팅 ① — 마이크로비트 파이썬 첫걸음」.
 *
 ...
 *   그 뒤 12·13차가 블록 코딩 진단활동 ①·②(seed-lesson12.ts·seed-lesson13.ts)로 채워지면서,
 *   마이크로비트 첫 시간은 13차 → 14차로 밀렸다가, **다시 15차로 한 칸 더 밀렸다**
 *   (LESSON_NO 12→13→14→15). 내용은 그대로다.
 *
 * ※ **왜 또 밀렸나(14→15):** 실제 보드/편집기 실습(이 파일) 앞에, 피지컬 컴퓨팅이 무엇인지
 *   개념부터 잡는 **입문 차시**(센서→처리→액추에이터)를 14차에 새로 넣었다
 *   (seed-lesson14.ts, activityId physical-computing-intro). 그 개념 입문이 이 하드코딩
 *   실습보다 앞에 오는 것이 자연스러워, 마이크로비트 실습을 14→15로 옮겼다. 아크는 여전히
 *   "피지컬 컴퓨팅 개념 입문(14) → 마이크로비트 파이썬(15) → 바이브 코딩 → 햄스터"다.
 *
 ...
 * 14차 개념 입문 통(physical-computing-intro)·디지털 윤리 통(digital-ethics)과 물리적으로
 * 다른 문서라 안 섞인다.
 */
