 ...
 * **입력(센서로 감지) → 처리(판단) → 출력(액추에이터로 반응)** 한 줄과, 주변에서 이미
  ...
  questions: [
    {
      ...
      answerIndex: 1,
      nowText:
        "센서로 감지하고(입력) → 컴퓨터가 판단하고(처리) → 액추에이터로 반응해요(출력). 자동문도 이 순서예요.",
      stickers: [],
    },
    {
    ...
    },
    {
      ...
      answerIndex: 1,
      nowText:
        "자동문: 센서가 사람을 감지하면(입력) → 판단해서(처리) → 모터가 문을 열어요(출력).",
      stickers: [],
    },
    {
      ...
      answerIndex: 1,
      nowText:
        "조도 센서가 어둠을 감지하면(입력) → 판단(처리) → 전등이 켜져요(출력). 켜지는 전등이 액추에이터예요.",
      stickers: [],
    },
    {
      prompt: "'센서로 감지 → 컴퓨터가 판단 → 액추에이터로 반응' 하는 것을 무엇이라고 부를까요?",
...
const WORKSHEET: WorksheetQuestion[] = [
  ...
  /* ── ② 피지컬 컴퓨팅 개념 note (입력=센서 → 처리 → 출력=액추에이터) ── */
  {
    ...
    label: "② 피지컬 컴퓨팅이 뭐예요?",
    hint:
      "'피지컬 컴퓨팅' 은 컴퓨터가 현실 세계를 **감지하고, 판단하고, 반응**하게 만드는 거예요.\n" +
      "순서는 딱 세 칸입니다:\n\n" +
      "  · 입력(감지) — 센서로 주변을 느껴요. (예: 밝기·온도·소리·거리·사람 움직임)\n" +
      "  · 처리(판단) — 컴퓨터가 '그러면 어떻게 할까' 를 정해요.\n" +
      ...
      "핵심 한 줄: **센서로 감지 → 판단 → 액추에이터로 반응.** 이 세 칸만 기억하면 돼요.\n" +
    ...
    tabs: [
      {
        ...
        rows: [
          { label: "한 줄로", value: "센서로 감지 → 컴퓨터가 판단 → 액추에이터로 반응" },
