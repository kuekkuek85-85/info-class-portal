/**
 * 「디지털 마음 톡톡」 4회기(7·8차시 통합, 실패를 노래로 자랑하기) 수업을 연다.
 *
 *   node --env-file=.env.local scripts/open-mt4.ts [날짜] [교시] [분반]
 *   예) node --env-file=.env.local scripts/open-mt4.ts 2026-09-15 7 mt-tue-1
 *
...
async function main(): Promise<void> {
  ...
  if (plans.empty) {
    console.error(`✗ ${LESSON_NO}차시 계획이 없습니다. 먼저 scripts/seed-mt4.ts 를 돌리세요.`);
    process.exit(1);
  }
  ...
  console.log(`  ${DATE} ${PERIOD}교시 · ${GROUP_LABEL} · 4회기 (실패를 노래로 자랑하기 · 7·8차시 통합)`);
