// ⚠ 임시 파킹 번호(LESSON_NO=123, 교사 확정 대기). 원래 15차 마이크로비트였으나,
//   교사가 커리큘럼 흐름을 바꿔(똥피하기 게임을 파이썬으로 직접 만든 뒤 마지막에 마이크로비트로
//   실물 제어) 앞자리(14·15)를 프로그래밍 언어 개론(14, seed-lesson14.ts)·똥피하기 분석·설계
//   (15, seed-lesson15.ts)로 채우면서 이 마이크로비트 실습을 뒤로 파킹했다.
//   ★ 게임 아크(파이썬 똥피하기: 분석·설계 + 구현 ≈ 7차시)와 피지컬 컴퓨팅 개념(122,
//     seed-lesson-pc-concept.ts) 뒤, **마이크로비트가 개념 바로 다음**에 온다.
//     아크 번호가 확정되면 122/123 을 실번호로 재배정한다. 그 전엔 재시드하지 않는다.
/**
 * (파킹) 마이크로비트 실습 차시 계획 등록 — 「피지컬 컴퓨팅 ① — 마이크로비트 파이썬 첫걸음」.
 *
 *   node --env-file=.env.local scripts/seed-lesson-microbit.ts
 *   node --env-file=.env.local scripts/seed-lesson-microbit.ts --force   (이미 학생이 들어온 수업도 덮어씀)
 *
 * ## 이 차시의 자리 (교사 확정 흐름)
 *
 * 아크: **프로그래밍 언어 개론+파이썬 맛보기(14) → 똥피하기 게임 분석·설계+구현(15~약7차시)
 *   → 피지컬 컴퓨팅 개념(122) → 마이크로비트 파이썬(이 차시, 123) → 바이브 코딩 → 햄스터**.
 * 익숙한 똥피하기를 파이썬(터틀)으로 직접 만든 뒤, 마지막에 마이크로비트로 실물 제어까지
 * 잇는다. 이 차시는 그 실물 제어 실습의 첫 시간 — 피지컬 컴퓨팅 개념(122) 바로 다음에 온다.
 * 게임 아크의 정확한 차시 수가 아직 안 정해져 **실번호가 미정**이라, 파이썬 도우미선발이
 * 121에 파킹된 관례처럼 개념(122)·마이크로비트(123)를 임시 파킹해 둔다. 내용은 그대로 보존.
 *
 * ## 프로그래밍·피지컬 컴퓨팅 단원의 실습 첫 시간이다 (디지털 문화 단원은 11차로 끝)
 *
 * 진단활동(12·13차)·개념 입문(14차)에 이어, 15차부터 마이크로비트 파이썬으로 실제 코딩을
 * 시작한다 (두 번째 큰 단원 **프로그래밍 + 피지컬 컴퓨팅**, 약 40차시, 12월 말까지).
 * 아크: **개념 입문(14차) → 마이크로비트 파이썬 → 바이브 코딩(포털 내부 Gemini) → 햄스터
 * 파이썬(수행평가2)**. 블록 코딩은 쓰지 않는다 — 파이썬 한 언어로 마이크로비트에서 햄스터까지 관통한다.
 *
 * 활동 통(activityId)은 `physical-computing` 이다 — 실제 보드/로봇 실습이 이어 쓰는 통.
 * 14차 개념 입문은 코드 기록이 없어 별도 통(physical-computing-intro)을 쓰므로, 이 실습
 * 통과 섞이지 않는다. 디지털 윤리 통(digital-ethics)도 재사용하지 않는다. 이후
 * 마이크로비트·햄스터 차시가 이 physical-computing 통을 이어 쓴다.
 *
...
const LESSON_NO = 123; // 임시 파킹 번호 (원래 15 — 똥피하기 분석·설계 15로 교체됨)

/** 9~11차시와 같은 규칙 — 아무도 안 들어온 수업에만 반영한다. --force 로 덮어쓸 수 있다 */
const FORCE = process.argv.includes("--force");

/**
 * **실습 활동 통.** 마이크로비트에서 햄스터까지 실제 보드/로봇 실습이 이어 쓴다 — 한 통.
 * 피지컬 컴퓨팅 개념 통(physical-computing-intro, 122)·똥피하기 게임 통(python-dodge-game)·
 * 디지털 윤리 통(digital-ethics)과 물리적으로 다른 문서라 안 섞인다.
 */
...
async function main(): Promise<void> {
  ...
  console.log(`\n활동 ID: ${ACTIVITY_ID} (실습 통 — 마이크로비트·햄스터 차시가 이어 씀. 14차 개념 입문 통 physical-computing-intro 과 분리)`);
