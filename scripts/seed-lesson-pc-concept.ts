// ⚠ 임시 파킹 번호(LESSON_NO=122, 교사 확정 대기). 원래 14차 「피지컬 컴퓨팅 개념」이었으나,
//   교사가 커리큘럼 흐름을 바꿔 14차를 「프로그래밍 언어 개론 + 파이썬 맛보기」(seed-lesson14.ts,
//   activityId python-intro)로 교체하면서 이 개념 차시를 뒤로 파킹했다.
//   ★ 게임 아크(파이썬 똥피하기: 분석·설계 + 구현 ≈ 7차시) 뒤·마이크로비트(seed-lesson15.ts,
//     LESSON_NO=123) 직전에 놓일 예정. 개념이 마이크로비트 바로 앞 순서다.
//     아크 번호가 확정되면 122/123 을 실번호로 재배정한다. 그 전엔 재시드하지 않는다.
/**
 * (파킹) 피지컬 컴퓨팅 개념 차시 계획 등록 — 「피지컬 컴퓨팅 개념 — 센서와 액추에이터」.
 *
 *   node --env-file=.env.local scripts/seed-lesson-pc-concept.ts
 *   node --env-file=.env.local scripts/seed-lesson-pc-concept.ts --force   (이미 학생이 들어온 수업도 덮어씀)
 *
 * ## 이 차시의 자리 (교사 확정 흐름: "개념은 뒤로")
 *
 * 교사가 아크를 바꿨다: 익숙한 똥피하기 게임을 파이썬(터틀/파이게임)으로 직접 만들며 기능을
 * 하나씩 붙이고, **마지막에 마이크로비트로 실물 제어**까지 잇는다. 그래서 순서는
 * **프로그래밍 언어 개론+파이썬 맛보기(14) → 게임 아크(파이썬 똥피하기: 분석·설계 + 구현 ≈ 7차시)
 *   → 피지컬 컴퓨팅 개념(이 차시) → 마이크로비트 파이썬(123)**.
 * 이 개념 차시는 게임 아크 뒤·마이크로비트 바로 앞에 온다 — 실물 제어 실습 직전에
 * "센서→처리→액추에이터" 감을 잡는 자리다. 게임 아크의 정확한 차시 수가 아직 안 정해져
 * **실번호가 미정**이라, 파이썬 도우미선발이 121에 파킹된 관례처럼 이 차시를 **122**에,
 * 마이크로비트를 **123**에 임시 파킹해 둔다. 아크 번호가 확정되면 실번호로 재배정한다.
 * (activityId physical-computing-intro, 내용 그대로 보존.)
 *
...
const LESSON_NO = 122; // 임시 파킹 번호 (원래 14 — 프로그래밍 언어 개론+파이썬 맛보기 14로 교체됨)
...
async function main(): Promise<void> {
  ...
  if (!existing.empty) {
    ...
    /*
     * 파킹 번호(122)라 보통은 기존 계획이 없어 아래 add 로 간다. 혹시 이전에 이 번호로
     * 시드된 것이 있어도, progress(다음 시간 탭)·progressChecks 가 merge 로 남지 않도록
